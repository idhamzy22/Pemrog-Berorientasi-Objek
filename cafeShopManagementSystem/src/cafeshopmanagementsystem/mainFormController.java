...
public class mainFormController implements Initializable {
    ...
    public void dashboardDisplayTI() {
        ...
        try {
            ...
            // Format ke Rupiah dengan NumberFormat
            NumberFormat formatRupiah = NumberFormat.getCurrencyInstance(new Locale("id", "ID"));
            formatRupiah.setMaximumFractionDigits(0); // Menghilangkan desimal
            dashboard_TI.setText(formatRupiah.format(ti));
    ...
    public void dashboardTotalI() {
        ...
        try {
            ...
            dashboard_TotalI.setText(formatRupiah.format(ti));
    ...
    public void menuDisplayTotal() {
        ...
        menu_total.setText(formatRupiah.format(totalP));
    }
    ...
    public void menuAmount() {
        // Ganti isBlank() dengan isEmpty() dan trim()
        if (menu_amount.getText() == null || menu_amount.getText().trim().isEmpty()) {
            alert = new Alert(AlertType.ERROR);
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Masukkan jumlah pembayaran!");
            alert.showAndWait();
            menu_change.setText("Rp0");
            return;
        }

        try {
            // Pastikan menggunakan nilai terbaru
            menuGetTotal();

            // Parse input amount dan hilangkan karakter non-digit
            String cleanAmount = menu_amount.getText().replaceAll("[^\\d]", "");
            ...
            if (amount <= 0) {
                ...
                alert.setTitle("Error Message");
                alert.setHeaderText(null);
                alert.setContentText("Jumlah pembayaran harus lebih dari 0!");
                alert.showAndWait();
                menu_change.setText("Rp0");
                return;
            }
            ...
            if (amount < totalP) {
                ...
                alert.setTitle("Error Message");
                alert.setHeaderText(null);
                alert.setContentText("Pembayaran kurang! Total: "
                        + NumberFormat.getCurrencyInstance(new Locale("id", "ID")).format(totalP));
                alert.showAndWait();
                menu_change.setText("Rp0");
                return;
            }
            ...
            NumberFormat formatRupiah = NumberFormat.getCurrencyInstance(new Locale("id", "ID"));
            formatRupiah.setMaximumFractionDigits(0);
            menu_change.setText(formatRupiah.format(change));

        } catch (NumberFormatException e) {
            alert = new Alert(AlertType.ERROR);
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Format pembayaran tidak valid!");
            alert.showAndWait();
            menu_amount.setText("");
            menu_change.setText("Rp0");
        }
    ...
    public void menuPayBtn() {
        if (totalP == 0) {
            ...
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Silakan pilih pesanan terlebih dahulu!");
            alert.showAndWait();
            return;
        }
        ...
        if (amount <= 0 || amount < totalP) {
            ...
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Pembayaran tidak valid atau kurang dari total!");
            alert.showAndWait();
            return;
        }
        ...
        try {
            ...
            alert.setHeaderText(null);
            alert.setContentText("Apakah Anda yakin ingin melakukan pembayaran?");
            Optional<ButtonType> option = alert.showAndWait();
            ...
            if (option.get().equals(ButtonType.OK)) {
                ...
                alert.setContentText("Pembayaran berhasil!\nTotal: "
                        + NumberFormat.getCurrencyInstance(new Locale("id", "ID")).format(totalP)
                        + "\nKembalian: "
                        + NumberFormat.getCurrencyInstance(new Locale("id", "ID")).format(change));
                alert.showAndWait();
    ...
    public void menuReceiptBtn() {
        if (totalP == 0 || menu_amount.getText().isEmpty()) {
            ...
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Please order first and complete the payment");
            alert.showAndWait();
            return;
        }

        // Dapatkan customer_id dari transaksi terakhir
        String getLastCustomerId = "SELECT customer_id FROM receipt ORDER BY id DESC LIMIT 1";
        int receiptCustomerId = 0;

        try {
            connect = database.connectDB();
            prepare = connect.prepareStatement(getLastCustomerId);
            result = prepare.executeQuery();

            if (result.next()) {
                receiptCustomerId = result.getInt("customer_id");
            }

            if (receiptCustomerId == 0) {
                alert = new Alert(AlertType.ERROR);
                alert.setTitle("Error Message");
                alert.setHeaderText(null);
            ...
            map.put("getReceipt", receiptCustomerId);

            // Format total ke Rupiah
            NumberFormat formatRupiah = NumberFormat.getCurrencyInstance(new Locale("id", "ID"));
            formatRupiah.setMaximumFractionDigits(0);
            map.put("formattedTotal", formatRupiah.format(totalP));

            // Load report design
            String reportPath = "src/cafeshopmanagementsystem/report.jrxml";
            ...
            if (!reportFile.exists()) {
                ...
                alert.setTitle("Error Message");
                alert.setHeaderText(null);
                alert.setContentText("Report template file not found at: " + reportPath);
                alert.showAndWait();
                return;
            }
            ...
            // Tampilkan preview receipt
            JasperViewer viewer = new JasperViewer(jPrint, false);
            viewer.setVisible(true);

            // Reset form setelah print receipt
            menuRestart();
            ...
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("Error generating receipt: " + e.getMessage());
            alert.showAndWait();
            e.printStackTrace();
        }
    }
